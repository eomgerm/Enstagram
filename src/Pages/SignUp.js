    ...
    >
      ...
      >
        <Container sx={{ display: "flex", justifyContent: "center", mt: "10px", mb: "15px" }}>
          <img src={require("../assets/images/Instagram_logo.svg").default} width="70%" alt="logo" />
        </Container>
        <Typography sx={{ color: "grey.600", textAlign: "center" }}>친구들의 사진과 동영상을 보려면</Typography>
        <Typography sx={{ color: "grey.600", textAlign: "center" }}>가입하세요.</Typography>
        <ThemeProvider theme={googleLoginBtn}>
          <Button onClick={handleClickGoogleLogin} sx={{ mt: "15px", mb: "15px" }} variant="contained">
            Google로 로그인
          </Button>
        </ThemeProvider>
        <Divider sx={{ mb: "10px" }}>
        ...
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <ThemeProvider theme={textInputTheme}>
              <TextField margin="dense" label="아이디" value={value} onChange={onChange} size="small" />
            </ThemeProvider>
          )}
        />
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <ThemeProvider theme={textInputTheme}>
              <TextField margin="dense" label="사용자 이름" value={value} onChange={onChange} size="small" />
            </ThemeProvider>
          )}
        />
        <Controller
          ...
          render={({ field: { onChange, value } }) => (
            <ThemeProvider theme={textInputTheme}>
              <TextField
                ...
                InputProps={{
                  endAdornment: (
                    <IconButton sx={{ position: "relative", right: "0px" }} onClick={handleClickShowPassword} edge="end">
